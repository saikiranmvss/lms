...
export const createNote = async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO notes (id, student_id, lesson_id, course_id, content, timestamp_seconds) VALUES ($1,$2,$3,$4,$5,$6)',
      [nid, req.user.id, lessonId, courseId, content, timestampSeconds || 0]
    );
    const result = await pool.query('SELECT * FROM notes WHERE id = $1', [nid]);
...
export const updateNote = async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE notes SET content = $1, updated_at = NOW() WHERE id = $2 AND student_id = $3',
      [content, id, req.user.id]
    );
    const result = await pool.query('SELECT * FROM notes WHERE id = $1 AND student_id = $2', [id, req.user.id]);
