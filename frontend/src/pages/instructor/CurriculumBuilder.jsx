...
const QUESTION_TYPES = [
  { value: 'mcq', label: 'Multiple Choice', icon: ListChecks, desc: 'One correct answer from several options' },
  { value: 'multiple_answer', label: 'Multiple Answer', icon: CheckSquare, desc: 'Multiple correct answers' },
  { value: 'true_false', label: 'True / False', icon: ToggleLeft, desc: 'Students choose true or false' },
    ...
    <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-slate-700">
              ...
              <span className="text-xs font-normal text-slate-500 ml-2">
                {form.type === 'mcq' ? '— select one correct' : '— select all correct'}
              </span>
            </label>
              ...
              return (
                <div key={i} className={clsx('flex items-center gap-2 p-2 rounded-lg border transition-all',
                  ...
                  <button
                    ...
                    title={form.type === 'mcq' ? 'Set as correct answer' : 'Toggle correct answer'}
                  ...
                  {form.options.length > 2 && (
                    <button type="button" onClick={() => removeOption(i)} className="text-slate-300 hover:text-red-400 transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
          ...
          {form.correctAnswers.length === 0 && (
            <p className="text-xs text-amber-600 mt-1.5 flex items-center gap-1">
              <Target className="w-3 h-3" /> Click the circle next to an option to mark it as correct
            </p>
          )}
        </div>
      )}
        ...
        <div>
          ...
          <div className="flex gap-3">
            {['True', 'False'].map(val => (
              ...
              >
                {form.correctAnswers.includes(val) && <Check className="w-4 h-4 inline mr-1.5" />}
                {val}
              </button>
            ))}
          </div>
      ...
      {form.type === 'short_answer' && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-sm text-amber-700 font-medium">Short Answer Question</p>
          <p className="text-xs text-amber-600 mt-0.5">Students will type their answer. This question type is not automatically graded.</p>
        </div>
      )}
...
function QuizBuilderModal({ lesson, courseId, onClose }) {
  ...
  const typeLabel = { mcq: 'MCQ', multiple_answer: 'Multi-Answer', true_false: 'True/False', short_answer: 'Short Answer' };
  const typeBadgeColor = { mcq: 'bg-blue-50 text-blue-600', multiple_answer: 'bg-violet-50 text-violet-600', true_false: 'bg-amber-50 text-amber-700', short_answer: 'bg-green-50 text-green-700' };
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className={clsx('badge text-xs px-2 py-0.5', typeBadgeColor[q.type] || 'bg-slate-100 text-slate-600')}>
                  ...
                  {q.type === 'short_answer' && (
                    <p className="mt-1 text-xs text-amber-600 italic">Open-ended — manually graded</p>
                  )}
          ...
          {addingQuestion ? (
            <div className="border-2 border-primary-200 rounded-xl p-5 bg-primary-50/30 space-y-4">
              ...
              <div className="flex gap-3 justify-end border-t border-slate-200 pt-4">
                <button onClick={() => { setAddingQuestion(false); setQuestionForm(emptyQuestion()); }} className="btn-secondary text-sm">
                  Cancel
                </button>
        ...
        <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5 flex items-center gap-1">
                <Target className="w-3.5 h-3.5 text-primary-500" /> Passing Score %
              </label>
              <input
                type="number"
                min="1"
                ...
                className="input"
              />
              <p className="text-xs text-slate-400 mt-1">Min score to pass</p>
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5 flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-primary-500" /> Time Limit (min)
              </label>
              <input
                type="number"
                min="1"
              ...
              />
              <p className="text-xs text-slate-400 mt-1">Leave blank for no limit</p>
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5 flex items-center gap-1">
                <RefreshCw className="w-3.5 h-3.5 text-primary-500" /> Max Attempts
              </label>
              <input
                type="number"
                min="1"
                ...
                className="input"
              />
              <p className="text-xs text-slate-400 mt-1">Retakes allowed</p>
            </div>
          </div>
          ...
          {quiz && (
            <div className="grid grid-cols-3 gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <div className="text-center">
                <p className="text-xl font-bold text-slate-900">{settings.passingPercentage}%</p>
                <p className="text-xs text-slate-500 mt-0.5">To Pass</p>
              </div>
            </div>
          )}
...
export default function CurriculumBuilder() {
  ...
  const createLessonMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (data.type === 'quiz') {
        try {
          await quizService.create({ lessonId: lesson.id, courseId, title: data.title, passingPercentage: 70, maxAttempts: 3 });
        } catch {}
      }
      return lessonRes;
    },
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          {course?.status === 'draft' && (
            <button onClick={() => publishMutation.mutate('pending')} disabled={publishMutation.isPending}
              className="btn-primary text-sm flex items-center gap-2">
              {publishMutation.isPending ? <RefreshCw className="w-4 h-4 animate-spin" /> : null}
              Submit for Review
            </button>
          )}
          {course?.status === 'pending' && (
            <span className="badge bg-yellow-100 text-yellow-700 px-3 py-1.5 text-sm">Under Review</span>
          )}
      ...
      <div className="flex items-center gap-6 px-4 py-3 bg-white rounded-xl border border-slate-200 text-sm text-slate-600">
        ...
        {course?.status && (
          <span className={clsx('ml-auto badge px-2.5 py-0.5 capitalize text-xs',
            ...
            course.status === 'pending' ? 'bg-yellow-50 text-yellow-700' :
      ...
      <div className="space-y-3">
        {(sections || []).length === 0 && (
          <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl">
            ...
            <p className="text-sm text-slate-400 mt-1">Create your first section to start building the curriculum</p>
          </div>
        )}
        ...
        {(sections || []).map((section, si) => (
          <div key={section.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="flex items-center gap-3 px-4 py-3.5 bg-slate-50 border-b border-slate-100">
              ...
              <button
                onClick={() => { setSectionForm({ title: section.title, description: section.description || '' }); setSectionModal({ open: true, editing: section }); }}
                className="text-slate-400 hover:text-primary-600 transition-colors p-1"
              >
                <Pencil className="w-4 h-4" />
              </button>
              <button
                onClick={() => { if (confirm(`Delete section "${section.title}" and all its lessons?`)) deleteSectionMutation.mutate(section.id); }}
            ...
            {openSections.includes(si) && (
              <div>
                {(section.lessons || []).length === 0 && (
                  <div className="px-6 py-4 text-sm text-slate-400 italic">No lessons yet</div>
                )}
                  ...
                  return (
                    <div key={lesson.id} className="flex items-center gap-3 px-5 py-3 border-b border-slate-50 hover:bg-slate-50/50 group">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {lesson.is_preview && (
                          <span className="badge bg-sky-50 text-sky-600 text-xs">Preview</span>
                        )}
                        {lesson.type === 'quiz' && (
                          <button
                ...
                <div className="px-5 py-3">
                  <button
                    onClick={() => { setLessonForm({ title: '', type: 'video', videoUrl: '', content: '', durationSeconds: '', isPreview: false }); setLessonModal({ open: true, sectionId: section.id }); }}
        ...
        <button
          onClick={() => { setSectionForm({ title: '', description: '' }); setSectionModal({ open: true, editing: null }); }}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="input"
              placeholder={lessonForm.type === 'quiz' ? 'e.g., Chapter 1 Quiz' : lessonForm.type === 'text' ? 'e.g., Introduction to React' : 'e.g., Setting up the project'}
              autoFocus
            />
          </div>
          ...
          {lessonForm.type === 'quiz' && (
            <div className="flex items-start gap-3 p-3 bg-purple-50 border border-purple-200 rounded-xl">
              ...
              <p className="text-sm text-purple-700">
                After creating the quiz lesson, click <strong>"Edit Quiz"</strong> next to it in the curriculum to add questions with multiple choice, true/false, and more.
              </p>
            </div>
          )}
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <button onClick={handleLessonSubmit} disabled={createLessonMutation.isPending} className="btn-primary flex items-center gap-2">
              {createLessonMutation.isPending ? <><RefreshCw className="w-4 h-4 animate-spin" /> Creating...</> : `Add ${lessonForm.type === 'quiz' ? 'Quiz' : lessonForm.type === 'text' ? 'Article' : 'Video'} Lesson`}
            </button>
          </div>
        </div>
      </Modal>
