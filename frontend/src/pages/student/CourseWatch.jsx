...
export default function CourseWatch() {
  ...
  const [aiMessages, setAiMessages] = useState([
    { role: 'assistant', content: 'Hi there! I am your AI Learning Tutor for this course. Feel free to ask me questions about the curriculum, lessons, or concepts you are learning!' }
  ]);
  ...
  const handleAiSubmit = async (e) => {
    e.preventDefault();
    if (!aiInput.trim() || aiLoading || !courseData?.course?.id) return;
    ...
    try {
      ...
      setAiMessages(prev => [...prev, { role: 'assistant', content: res.data.data.reply }]);
    } catch (err) {
      toast.error('AI tutor did not respond. Please try again.');
      setAiMessages(prev => [...prev, { role: 'assistant', content: '⚠️ Sorry, I encountered an error connecting to my database. Please try asking again in a moment.' }]);
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const progressMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['course-watch'] });
      refetchProgress();

      // Refresh the user session stats (XP, streaks, badges) in the local store
      authService.getMe().then((fresh) => {
        setUser(fresh.data.data);
      }).catch(() => {});

      // Alert if a new badge was unlocked
      const data = res.data.data;
      if (data?.unlockedBadge) {
        toast.success(`🎉 Badge Unlocked: ${data.unlockedBadge}! Check your profile.`);
      }
    },
  });
  ...
  const completedIds = new Set((progressData?.progress || []).filter(p => Number(p.is_completed) === 1).map(p => p.lesson_id));
                ...
                >
                  {activeTab === 'ai-tutor' && (
                    <div className="space-y-4">
                      <div className="flex flex-col rounded-2xl border border-slate-200/70 bg-white shadow-sm overflow-hidden h-[500px]">
                        {/* Chat messages */}
                        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
                          {aiMessages.map((msg, idx) => (
                            <div
                              key={idx}
                              className={clsx(
                                'flex gap-3 max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed shadow-[0_1px_2px_rgba(0,0,0,0.02)]',
                                msg.role === 'assistant'
                            ...
                            >
                              {msg.role === 'assistant' && (
                                <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-indigo-50 text-indigo-600 mr-1 mt-0.5">
                          ...
                          {aiLoading && (
                            <div className="flex gap-3 max-w-[85%] rounded-2xl p-4 text-sm bg-white text-slate-800 border border-slate-200/80 mr-auto">
                              <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-indigo-50 text-indigo-600 mr-1 mt-0.5">
                        ...
                        {/* Input form */}
                        <form onSubmit={handleAiSubmit} className="border-t border-slate-200 p-4 bg-white flex gap-2">
                          <input
                            type="text"
                            value={aiInput}
                            onChange={(e) => setAiInput(e.target.value)}
                            placeholder={aiLoading ? 'AI is thinking...' : 'Ask about this lesson or course concepts...'}
                            disabled={aiLoading}
                            className="flex-1 input border-slate-200 bg-slate-50/80 focus:bg-white text-sm"
                          />
                          <button
                            type="submit"
                          ...
                          <button
                            ...
                            onClick={() => setAiMessages([
                              { role: 'assistant', content: `I've cleared the previous history. Let's start fresh! What would you like to know about "${course.title}"?` }
                            ])}
